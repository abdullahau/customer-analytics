...
-   In Stan, we will us the expression @eq-numtrans and @eq-numtrans2 given the nature of the dropout process and the time between transactions:
...
-   We will also compute the number of transactions for each customer using the expression @eq-numtrans3.

```{python}
# probability of observing x purchases in a time period of length t
def pmf(x, t, r, alpha, a, b):
    A = np.cumsum(
...
def bgnbd_predict_trans_dist(x, n, T, r, alpha, a, b):
    pmf = bgnbd_pmf(x, T, r, alpha, a, b)
...
We will implement the @eq-P-alive-T as a Python function:
...
def expected_probability_y_purchase(x, t_x, T, y, t, r, alpha, a, b):
    ...
    # Compute denominator Log-Likelihood
    L_term1 = (
        (beta(a, b + x) / beta(a, b)) * 
        (gamma(r + x) / gamma(r)) * 
        (alpha**r / (alpha + T)**(r + x))
...
Let us look into the probability of a customer making $0, 1, 2, 3, 4, \text{ or } 5$ number of purchases between now, and 30 days from now.
