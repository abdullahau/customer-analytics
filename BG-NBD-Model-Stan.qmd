...
& \qquad \frac{\frac{a+b+x-1}{a-1}\left[1-\left(\frac{\alpha+T}{\alpha+T+t}\right)^{r+x}{ }_{2} F_{1}\left(r+x, b+x ; a+b+x-1 ; \frac{t}{\alpha+T+t}\right)\right]}{1+\delta_{x>0} \frac{a}{b+x-1}\left(\frac{\alpha+T}{\alpha+t_{x}}\right)^{r+x}} \tag{10}
\end{align*}
$$
...
params = [post_preds.stan_variable('r').reshape(-1,1) for param in ['r', 'alpha', 'a', 'b']]
exp_purch = expected_purchases(p1x_sample, t_x_sample, T_sample, future_t.reshape(-1,1,1), *params)
...
for i, cust_id in enumerate(example_customer_ids):
    ...
    ax.plot(future_t, mean_purchases, color='blue', label='Mean')
    ax.fill_between(future_t, pi[0,:], pi[1,:], color='blue', alpha=0.3, label='89% PI')
