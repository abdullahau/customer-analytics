...
# '..', '..' moves up two directory levels (lib/utils -> repo root) to reach data/
...
class CDNOW(RFM):
    def __init__(
        self, master: bool = True, calib_p: int = 273, remove_unauthorized=False
    ):
        # Select and preprocess the data
        data = self.__select_data(master=master)
        super().__init__(
            data=data, dateformat="%Y%m%d", calib_p=calib_p
