...
class RFM(object):
    def __init__(
        self, data: Union[pl.LazyFrame, pl.DataFrame], dateformat: str, calib_p: int
    ) -> None:
        """
    ...
    def __data_prep(self, data):
        ...
        """
        return (
            data.with_columns(pl.col("Date").str.to_date(self.dateformat))
            .with_columns(
                ((pl.col("Date") - pl.col("Date").min()).dt.total_days() + 1)
                .cast(pl.UInt16)
                .alias("PurchDay")
            )
            .with_columns(
                (pl.col("Spend") * 100).round(0).cast(pl.Int64).alias("Spend Scaled")
            )
            .group_by("ID", "Date")
            .agg(
    ...
    def rfm_summary(self) -> Union[pl.LazyFrame, pl.DataFrame]:
        ...
        rfm_data = m_x.join(
            other=ttlrp, on="ID", how="left"
        ).with_columns(
            pl.when(pl.col("P1X") > 0)
            .then(
                pl.col("P1X Spend") / pl.col("P1X")
            )  # average spend per repeat transaction
            .otherwise(0)
            .alias(
                "zbar"
            )  # customer’s observed average transaction value, an imperfect estimate of their (unobserved) mean transaction value
        )
    ...
    def frequency(self) -> Union[pl.LazyFrame, pl.DataFrame]:
        ...
        freq_x = self.data.group_by("ID", maintain_order=True).agg(
            pl.col("PurchDay")
    ...
    def avg_spend(
        self,
        pSpendQuant: Union[pl.LazyFrame, pl.DataFrame],
        freq_x: Union[pl.LazyFrame, pl.DataFrame],
    ) -> Union[pl.LazyFrame, pl.DataFrame]:
        "Monetary Value (m_{x}) - The average spend per repeat transaction for the calibration and validation periods"
        m_x = (
            pSpendQuant.join(freq_x, on="ID", how="left")
            .with_columns(
    ...
    def t_x_T(
        ...
        ttlrp = (
            ...
            .with_columns(
                ...
                ((self.calib_p - pl.col("Trial Day")) / 7).alias(
                    "T"
    ...
    def repeat_sales(self) -> pl.DataFrame:
        ...
        return pl.concat(
            [
                pl.DataFrame(
                    {"Week": 1, "Weekly Sales": 0, "Cum Sales": 0},
                    schema={
                        "Week": pl.UInt16,
                        "Weekly Sales": pl.Float32,
                        "Cum Sales": pl.Float32,
                    },
                ),
                data,
            ]
    ...
    def repeat_frequency(self, calib=True):
        ...
        return (
            freq_x.group_by(period).agg(pl.len().alias("Repeat Frequency")).sort(period)
        )
