...
__all__ = ["ChartTemp", "layered_line_prop"]
...
class ChartTemp(alt.Chart):
    ...
    def line_encode(
        ...
        line = self.mark_line(strokeWidth=2, strokeDash=dash).encode(
            ...
            y=alt.Y(y_col, title=y_title, scale=y_scale),
            color=color,
        )
        return line
    ...
    def line_prop(self, title):
        line = (
            self.properties(width=650, height=250, title=title)
...
def layered_line_prop(chart, title):
    line = (
        chart.properties(width=650, height=250, title=title)
        .configure_view(stroke=None)
        .configure_axisY(grid=False)
        .configure_axisX(grid=False)
    )
