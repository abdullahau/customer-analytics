...
code-links:
  - text: View source
    icon: file-code
    href: https://github.com/abdullahau/customer-analytics/blob/main/notebooks/analyses/buyer-behavior-summary-panel-data.qmd
---
...
*Note on `price`*: it is deliberately read as `Float64` rather than `Float32`. Spend is `units × price` accumulated over thousands of rows, and the decile analyses below rank households by total spend and then cut on a **cumulative** sum. In `Float32` that running total loses enough precision that genuinely distinct household spends collapse into ties, and households straddling a decile boundary get assigned to the wrong decile — the household counts still look right, but the per-decile transaction and brand averages silently drift.
...
def weekly_vol_summary(brand, lf):
    ...
    if brand != 'All':
        brands = [brand] if isinstance(brand, str) else list(brand)
        lf = lf.filter(pl.col('brand').is_in(brands))

    return (
        lf
        .with_columns((pl.col('units') * pl.col('weight') / 1000).alias('volume'))
        .group_by('week', 'brand')
        .agg(pl.col("volume").sum().alias('Weekly Volume'))
        .sort('week')
    )
```
...
```{python}
exceptions = [
    'grocery_lf', 'sku_lf', 'kiwi_lf', 'BRANDS', 'panel_size',
    'panellist_trans', 'panellist_spend', 'panellist_vol', 'panellist_category',
    'penetration', 'ppb', 'decile_base', 'decile_table',
...
![](/references/figures/intermediate-data-prep.png){fig-align="center" width="734"}

### Panellist-Level Data Preparation

![](/references/figures/Consumer-Panel-Data-Relationship.png){fig-align="center" width="605"}

The Excel note gets there in two hops: first collapse the raw purchase lines to **one row per transaction** carrying a binary indicator of which brands were bought, then sum those indicators per panellist. In polars the same thing is one step — count *distinct* `trans_id`s within each `panel_id × brand` group — which is why the code below has no intermediate indicator table. The distinction that matters is the same either way: we are counting purchase **occasions**, not units, and not raw rows (a single transaction contributes several rows when the checkout scanned items separately).

**The three panellist-level summaries**

Every analysis in the rest of this note is built from just three tables, each one row per panellist and one column per brand (plus a `Category` column):

| Table | Cell value |
|---|---|
| `panellist_trans` | number of **purchase occasions** on which the brand was bought |
| `panellist_spend` | total **dollars** spent on the brand |
| `panellist_vol`   | total **kilograms** of the brand purchased |

These are exactly the `panellist x brand (trans)`, `(spend)` and `(volume)` worksheets of the Excel note, and the `df_panellist_trans`, `df_panellist_spend` and `df_panellist_vol` data frames of the R note.

Two details matter:

-   **Transactions are not additive across brands.** A brand transaction is a purchase *occasion* on which at least one item of that brand was bought, so it is a count of distinct `trans_id`s — not a sum of units. `Category` is likewise the count of distinct `trans_id`s, which is why the brand columns can sum to *more* than `Category`: panellist `3102016`'s transaction `844` is one category occasion but counts once for Alpha and once for Delta. Spend and volume, by contrast, are plain sums, so there `Category` is exactly the row total.
-   **All three tables share the same `panel_id` ordering**, so they can be lined up positionally (as the matrix algebra later on requires) without re-joining.

Building all three in a single pass over the data matters: the earlier version of this note re-scanned and re-collected the CSV once per call, more than a dozen times over.

```{python}
BRANDS = ['Alpha', 'Bravo', 'Charlie', 'Delta', 'Other']


def panellist_matrices(lf, year=1):
    """Panellist x brand transaction, spend and volume matrices for a given year.

    Returns three DataFrames sharing an identical `panel_id` ordering, each with
    one column per brand plus a `Category` column.
    """
    period = (
        lf
        .filter(pl.col('week').is_between((year - 1) * 52 + 1, year * 52))
        .join(sku_lf, on='sku_id')
        .with_columns(
            (pl.col('units') * pl.col('price')).alias('spend'),
            # weight from grams to kilograms
            (pl.col('units') * pl.col('weight') / 1000).alias('volume'),
        )
        .collect()
    )

    def wide(frame, values, aggregate_function):
        """Pivot a long panel_id x brand aggregate into a brand-per-column table."""
        return (
            frame
            .pivot(on='brand', index='panel_id', values=values,
                   aggregate_function=aggregate_function)
            .select(['panel_id'] + BRANDS)
            .fill_null(0)
            .sort('panel_id')
        )

    # Transactions: distinct purchase occasions, so brand columns are NOT additive
    # and `Category` has to be counted separately rather than summed across brands.
    trans = (
        wide(
            period.group_by('panel_id', 'brand').agg(pl.col('trans_id').n_unique()),
            values='trans_id', aggregate_function='first',
        )
        .join(
            period.group_by('panel_id').agg(pl.col('trans_id').n_unique().alias('Category')),
            on='panel_id',
        )
        .sort('panel_id')
    )

    # Spend and volume are plain sums, so `Category` is the row total.
    spend, volume = (
        wide(period, values=value, aggregate_function='sum')
        .with_columns(pl.sum_horizontal(BRANDS).alias('Category'))
        for value in ('spend', 'volume')
    )

    return trans, spend, volume


panellist_trans, panellist_spend, panellist_vol = panellist_matrices(grocery_lf, year=1)

panellist_trans.head()
```

```{python}
panellist_spend.head()
```
...
A fourth, derived table is convenient often enough to be worth naming: each category buyer's total purchase occasions alongside how many *distinct* brands they bought.

```{python}
panellist_category = panellist_trans.select(
    'panel_id',
    pl.col('Category').alias('# of Purchases'),
    pl.sum_horizontal(pl.col(BRANDS) > 0).alias('Brands Purchased'),
)

panellist_category.head()
```
...
buyers = panellist_trans.select(
    (pl.col(BRANDS + ['Category']) > 0).sum()
).unpivot(
    variable_name='brand',
    value_name='buyers'
)
...
transactions = panellist_trans.select(
    pl.col(BRANDS + ['Category']).sum()
).unpivot(
    variable_name='brand',
...
alpha_trans_dist = (
    ...
    .group_by(pl.col('Alpha').alias('# of Purchases'))
    .agg(pl.len().alias("Frequency"))
    .pipe(
        lambda df: df.vstack(
            pl.DataFrame(
...
```{python}
category_spend = panellist_spend.select(pl.col('Category').alias('spend'))
...
```{python}
alt.Chart(category_spend).transform_density(
    'spend',
    as_=['spend', 'Density']
).mark_area().encode(
...
percentile_values = np.percentile(category_spend['spend'], percentiles)
...
```{python}
alpha_spend = panellist_spend.filter(pl.col('Alpha') > 0).select(pl.col('Alpha').alias('spend'))
...
percentile_values = np.percentile(alpha_spend['spend'], percentiles)
...
one_time_alpha_buyers = panellist_trans.filter(pl.col('Alpha') == 1).select('panel_id')
one_time_alpha_buyers

# Alpha buyers who spent between $2 to $4
spend_range_alpha_buyers = panellist_spend.select(
    'panel_id', pl.col('Alpha').alias('spend')
).filter(
    (pl.col('spend') >= 2) &
    (pl.col('spend') < 4)
)

res = one_time_alpha_buyers.join(
    spend_range_alpha_buyers, on='panel_id', how='semi'
).height
...
```{python}
spend_range_alpha_buyers.join(
    one_time_alpha_buyers.select('panel_id'), on='panel_id', how='semi'
...
grocery_lf.filter(
    ...
    spend_range_alpha_buyers.lazy().select('panel_id'), on='panel_id', how='semi'
).join(
    one_time_alpha_buyers.lazy(), on='panel_id', how='semi'
).with_columns(
    ((pl.col('units') * pl.col('price'))).alias('spend')
...
```{python}
category_vol = panellist_vol.select(pl.col('Category').alias('volume'))
...
percentile_values = np.percentile(category_vol['volume'], percentiles)
...
```{python}
alpha_vol = panellist_vol.filter(pl.col('Alpha') > 0).select(pl.col('Alpha').alias('volume'))
...
percentile_values = np.percentile(alpha_vol['volume'], percentiles)
...
Both versions rank the same set of households, so we start from a single base table: one row per category buyer with their total category spend, their category transactions, and the number of distinct brands they bought.

The **ordering must be deterministic**. 2,548 of the 4,574 households share a total spend with at least one other household, and both decile definitions cut on a *cumulative* quantity, so households sitting on a decile boundary are decided by the tie-break. Sorting on spend alone leaves that tie order arbitrary — polars does not guarantee a stable sort — and the table then changes from run to run. Sorting by `spend` descending **and then `panel_id` ascending** fixes the order and reproduces the R note exactly.

```{python}
decile_base = (
    panellist_spend.select('panel_id', pl.col('Category').alias('spend'))
    .join(panellist_category, on='panel_id')
    # deterministic tie-break: highest spend first, then lowest panel_id
    .sort(['spend', 'panel_id'], descending=[True, False])
)


def decile_table(base):
    """Summarise buying behaviour by decile, given a frame with a `decile` column."""
    return (
        base
        .group_by('decile')
        .agg(
            pl.len().alias('HH'),
            pl.col('spend').sum(),
            pl.col('# of Purchases').sum().alias('trans'),
            pl.col('Brands Purchased').mean().alias('# Unique Brands'),
        )
        .sort('decile')
        .with_columns(
            (pl.col('HH') / pl.col('HH').sum()).alias('% HH'),
            (pl.col('spend') / pl.col('spend').sum()).alias('% Spend'),
            (pl.col('trans') / pl.col('trans').sum()).alias('% Trans'),
            (pl.col('spend') / pl.col('HH')).alias('Spend/HH'),
            (pl.col('trans') / pl.col('HH')).alias('Cat Trans/HH'),
            (pl.col('spend') / pl.col('trans')).alias('Avg Spend/Trans'),
        )
        .select('decile', '% HH', '% Spend', '% Trans', 'Spend/HH',
                'Cat Trans/HH', 'Avg Spend/Trans', '# Unique Brands')
        .rename({'decile': 'Decile'})
    )
```

Deciles of equal *size*: rank the sorted households and split into ten groups of 10% of buyers each.

```{python}
buyer_decile = decile_base.with_columns(
    ((pl.int_range(pl.len()) * 10) // pl.len() + 1).alias('decile')
)

decile_tab = decile_table(buyer_decile)
decile_tab
```
...
```{python}
# Assign each household to the decile of *cumulative* category spend it falls in.
# `cum` is the running share of total category revenue (0-100%); the 1e-6 nudge
# matches the R note and keeps a household landing exactly on a boundary in the
# decile above it.
spend_total = decile_base['spend'].sum()

spend_decile = (
    decile_base
    .with_columns((100 * pl.col('spend').cum_sum() / spend_total).alias('cum'))
    .with_columns(
        (((pl.col('cum') - 1e-6) / 10).floor() + 1)
        .clip(1, 10)
        .cast(pl.UInt16)
        .alias('decile')
    )
...
decile_tab_spend = decile_table(spend_decile)
decile_tab_spend
```
...
alpha_lorenz_plot = alpha_trans_dist.select('Cum % of Buyers' ,'Cum % Transactions')
...
![](/references/figures/linear_interp1.png){width="200"}
:::

::: {style="max-width:200px;margin-left: auto; margin-right: auto;"}
![](/references/figures/linear_interp.png){width="200"}
:::
...
alpha_spend_dist = (
    panellist_spend
    .select('panel_id', pl.col('Alpha').alias('spend'))
...
```{python}
brand_purchases = panellist_trans.select(BRANDS).to_numpy()

# Step 1: Create ever buyers matrix
ever_buyers = (brand_purchases > 0).astype(int)

# Step 2: Compute duplication count matrix
dup_count = ever_buyers.T @ ever_buyers
brand_buyers = np.diag(dup_count).reshape(-1, 1)
...
dup_count_df = (
    pl.from_numpy(dup_count, schema=BRANDS)
    .with_columns(pl.Series('Brands', BRANDS))
)

dup_purchase_df = (
    pl.from_numpy(dup_purchase, schema=BRANDS)
    .with_columns(pl.Series('Brands', BRANDS))
)
```
...
```{python}
# SCR falls straight out of the "ever buyers" matrix: pre-multiplying the
# panellist x brand *volume* table by the transpose of the incidence matrix gives,
# for each row brand, the category volume bought by that brand's buyers.
brand_vol = panellist_vol.select(BRANDS).to_numpy()
ever_buyers = (brand_vol > 0).astype(int)

cross_purch_matrix = ever_buyers.T @ brand_vol

scr = pl.DataFrame({
    'Brands': BRANDS,
    # Total purchasing of each brand
    'Brand Purchasing': np.diag(cross_purch_matrix),
    # Category purchasing conditioned on having bought the brand at least once
    'Category Purchasing': cross_purch_matrix.sum(axis=1),
}).with_columns(
    (pl.col('Brand Purchasing') / pl.col('Category Purchasing')).alias('SCR')
)
...
```{python}
brand_vol_purchases = panellist_vol.select(BRANDS).to_numpy()

# Step 1: Create ever buyers matrix
ever_buyers = (brand_vol_purchases > 0).astype(int)

# Step 2: Compute cross purchase matrix
cross_purch_matrix = ever_buyers.T @ brand_vol_purchases
total_volume_purchases = cross_purch_matrix.sum(axis=1).reshape(-1, 1)
...
cross_purch_matrix_df = (
    pl.from_numpy(cross_purch_matrix, schema=BRANDS)
    .with_columns(pl.Series('Brands', BRANDS))
)

cross_purch_df = (
    pl.from_numpy(cross_purch, schema=BRANDS)
    .with_columns(pl.Series('Brands', BRANDS))
)
```
...
```{python}
cross_purch_plot = (
    pl.from_numpy(cross_purch.T, schema=BRANDS, orient='row')
    .with_columns(pl.Series('Brands', BRANDS))
)
...
# Market Share - Volume
market_share = brand_vol_purchases.sum(axis=0) / brand_vol_purchases.sum()
...
```{python}
brand_spend_purchases = panellist_spend.select(BRANDS).to_numpy()

# Step 1: Create ever buyers matrix
ever_buyers = (brand_spend_purchases > 0).astype(int)

# Step 2: Compute cross purchase matrix
cross_purch_matrix = ever_buyers.T @ brand_spend_purchases
total_spend_purchases = cross_purch_matrix.sum(axis=1).reshape(-1, 1)
...
# Market Share - Volume
market_share = brand_spend_purchases.sum(axis=0) / brand_spend_purchases.sum()
...
```{python}
# Marginal distribution: the % of panellists making x transactions, per year.
# Built once in long form, then pivoted for the table.
marginal_dist = (
    pl.concat([
        joint_dist_trans
        .group_by(year)
        .agg((pl.col('count').sum() / panel_size).alias('% of Total'))
        .with_columns(pl.lit(year).alias('Year'))
        .rename({year: 'num_trans'})
        for year in ('Year 1 Trans', 'Year 2 Trans')
    ])
    .sort('Year', 'num_trans')
)

marginal_dist.pivot(on='Year', index='num_trans', values='% of Total').sort('num_trans')
```
...
alpha_qtrly_price_avg = (
    ...
    .group_by('quarter')
    # average price per kg is total revenue / total volume for the quarter -- the
    # same definition used for the weekly and annual decompositions above. Taking
    # the mean of per-line price/kg ratios instead would weight a single-unit
    # purchase the same as a bulk one and does not reconcile with revenue.
    .agg((pl.col('spend').sum() / pl.col('volume').sum()).alias('avg_price_per_kg'))
    .sort('quarter')
    .collect()
)
...
missing_weeks = sorted(set(range(1, 53)) - set(kiwi_df['Week'].to_list()))
print(missing_weeks)
```
...
missing_df = pl.DataFrame(
    {col: [None]*len(missing_weeks) for col in kiwi_df.columns}
        ...
        pl.Series('Week', values=missing_weeks, dtype=kiwi_df.schema['Week'])
    )
