...
res = one_time_alpha_buyers.with_columns(
    contains=pl.col('panel_id').is_in(
        spend_range_alpha_buyers.get_column('panel_id')
    )
).select('contains').sum().item()

# res = (
#     one_time_alpha_buyers.lazy()
#     .join(spend_range_alpha_buyers.lazy(), on='panel_id', how='semi')
#     .select(pl.len())
#     .collect()
#     .item()
# )
...
spend_range_alpha_buyers.filter(
    pl.col('panel_id').is_in(one_time_alpha_buyers.get_column('panel_id'))
...
grocery_lf.filter(
    (pl.col('week') <= 52) &
    (pl.col('panel_id').is_in(spend_range_alpha_buyers.get_column('panel_id'))) &
    (pl.col('panel_id').is_in(one_time_alpha_buyers.get_column('panel_id'))) &
    (pl.col('brand') == 'Alpha')
).with_columns(
    ((pl.col('units') * pl.col('price'))).alias('spend')
