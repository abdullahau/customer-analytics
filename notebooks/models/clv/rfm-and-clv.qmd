...
code-links:
  - text: View source
    icon: file-code
    href: https://github.com/abdullahau/customer-analytics/blob/main/notebooks/models/clv/rfm-and-clv.qmd
---
...
X = np.arange(39) + 1                    # recency
Y = np.arange(8)                         # frequency
Z = plot_data[:, 1:].to_numpy().T / 100  # spend  
Z[:, 0] = 0
```
...
for Yi in range(8):
    ax.fill_between(
        X, Yi, Z[:, Yi], X, Yi, 0, facecolors="white", edgecolor="black", linewidth=0.5
    )

ax.set_xlabel("Recency ($t_{x}$)")
...
ax.set_xlim(0, 40)     # recency on x
ax.set_ylim(0, 7)      # frequency on y
ax.set_zlim(-5, 400)   # spend on z

for a in (ax.xaxis, ax.yaxis, ax.zaxis):
    a.pane.fill = False
    a._axinfo["grid"].update(linestyle=(0, (1, 5)), color="gray")
ax.grid(True)

ax.set_box_aspect(None, zoom=0.82)
ax.set_yticks(Y, labels=[str(i) if i < 7 else "7+" for i in range(8)])
...
cs = plt.contour(X, Y, Z.T, levels=[50, 100, 200, 300], linewidths=0.75, colors="black")
...
def paretonbd_E_X_t(t, r, alpha, s, beta_param):
    ...
    central to computing the expected transaction volume fore whole customer base over time
    """
    return (
        r
        * beta_param
...
```{python}
t_x_range = np.arange(79).reshape(-1, 1) # Recency
x_range = np.arange(15)                  # Frequency
...
ax.plot_wireframe(
    t_x_range, x_range, DET_Z, rstride=0, cstride=1, color="black", linewidth=0.75
)
ax.set_xlabel("Recency ($t_{x}$)")
ax.set_ylabel("Frequency ($x$)")
ax.zaxis.set_rotate_label(False)
...
# 3D “waterfall” plot
fig, ax = plt.subplots(1, 2, figsize=(10, 8), dpi=50, subplot_kw={"projection": "3d"})
for i, sub in enumerate(ax):
    ...
    sub.plot_wireframe(
        t_x_range, x_range, CLV_Z, rstride=0, cstride=1, color="black", linewidth=0.75
    )
    ...
    sub.view_init(elev=25, azim=-135, roll=0)
    for a in (sub.xaxis, sub.yaxis, sub.zaxis):
        a.pane.fill = False
        a._axinfo["grid"].update(linestyle=(0, (1, 5)), color="gray")
    sub.grid(True)
...
```{python}
# Iso-Value Representation of DET
fig, ax = plt.subplots(1, 2, figsize=(10, 5.5), dpi=50)
for i, sub in enumerate(ax):
    CLV_Z = CLV(
        cm,
        delta,
...
for Yi in range(1, 21):
    ax.fill_between(
        X, Yi, Z[:, Yi - 1], X, Yi, 0, facecolors="white", edgecolor="black", linewidth=0.5
    )
