...
code-links:
  - text: View source
    icon: file-code
    href: https://github.com/abdullahau/customer-analytics/blob/main/notebooks/models/purchasing/bg-nbd-stan.qmd
---
...
![](/references/figures/recency_frequency_timeline.jpg){fig-align="center" width="342"}
...
pc = az.plot_trace_dist(
    inf_data, var_names=["r", "alpha", "a", "b", "phi_dropout", "kappa_dropout"], backend="matplotlib"
)
fig = pc.viz["figure"].item()
fig.set_size_inches(12, 9)
fig.suptitle("BG/NBD Model Trace", fontsize=18, fontweight="bold")
plt.tight_layout();
```
...
for i, var_name in enumerate(["r", "alpha", "a", "b"]):
    ax = pc.get_target(var_name, {})
    ax.axvline(
        stan_model.opt_model.stan_variable(var_name),
        color="C1",
        linestyle="--",
        label="Stan MAP",
    )
    ax.axvline(lifetimes_params[i], color="C2", linestyle="--", label="Lifetimes")
    ax.axvline(scipy_params[i], color="C3", linestyle="--", label="SciPy")
    ax.legend(loc="upper right", fontsize=8)

fig = pc.viz["figure"].item()
fig.set_size_inches(12, 9)
fig.suptitle("BG/NBD Model Parameters", fontsize=18, fontweight="bold");
```
...
stan_model = Stan(stan_file="stan/src/bg-nbd-prior", stan_code=stan_prior_code)
prior_fit = stan_model.sample(data=data)
```

```{python}
# Drop any non-finite draws (the heavy-tailed pareto(1,1) prior on
# kappa_dropout occasionally yields a degenerate, all-NaN simulated draw).
X_rep = prior_fit.stan_variable("X_rep")
prior_samples = X_rep[np.isfinite(X_rep).all(axis=1)].astype(int)
...
hdi = az.hdi(prior_freq_counts.T, prob=0.89)
...
```{python}
max_purch = 7
prior_draws = {p: prior_fit.stan_variable(p) for p in ["r", "alpha", "a", "b"]}
finite = np.all([np.isfinite(v) for v in prior_draws.values()], axis=0)
params = [prior_draws[p][finite].reshape(1, 1, -1) for p in ["r", "alpha", "a", "b"]]
...
hdi = az.hdi(frequency_counts, prob=0.89)
...
stan_model = Stan(stan_file="stan/src/bg-nbd-post", stan_code=stan_post_code)
post_preds = stan_model.sample(data=data)
```

```{python}
posterior = az.from_cmdstanpy(post_preds, posterior_predictive=["X_rep", "t_x_rep"])
X_rep = post_preds.stan_variable("X_rep")
posterior_samples = X_rep[np.isfinite(X_rep).all(axis=1)].astype(int)
...
hdi = az.hdi(frequency_counts.T, prob=0.89)
...
```{python}
max_purch = 7
post_draws = {p: post_preds.stan_variable(p) for p in ["r", "alpha", "a", "b"]}
finite = np.all([np.isfinite(v) for v in post_draws.values()], axis=0)
params = [post_draws[p][finite].reshape(1, 1, -1) for p in ["r", "alpha", "a", "b"]]
...
Now that we have fitted the model, we can use it to make predictions. For example, we can predict the expected probability of a customer being alive as a function of time.
...
We will use the forward-looking @eq-E-Y-t to obtain the expected number of transactions in a future period of length $t$ for an individual with past observed behavior $\left(X=x, t_{x}, T\right)$.
...
params = [
    post_preds.stan_variable(param).reshape(-1, 1) for param in ["r", "alpha", "a", "b"]
]
exp_purch = expected_purchases(
    p1x_sample, t_x_sample, T_sample, future_t.reshape(-1, 1, 1), *params
)  # shape (91, 4000, 6)

fig, axes = plt.subplots(
    nrows=6, ncols=1, figsize=(10, 15), sharex=True, sharey=True, layout="constrained"
)
...
We will implement @eq-P-alive-T as a Python function:
...
future_alive = expected_probability_alive(
    p1x_sample, t_x_sample, (T_sample + future_t.reshape(-1, 1, 1)), *params
)
...
def expected_probability_y_purchase(x, t_x, T, y, t, r, alpha, a, b):
    ...
    if y > 0:
        ...
        C_part = (beta(a + 1, b + x + y - 1) / beta(a, b)) * (
...
axes[-1].set_xlabel("Future Time Period")
plt.gcf().suptitle(
    "Expected Probability of Zero Purchases between $(T, T+t]$.", fontweight="bold"
)
plt.show()
```

### Probability of a Customer Making $y$ Purchases Within 30 days from Now

Let us look into the probability of a customer making $0, 1, 2, 3, 4, \text{ or } 5$ purchases between now and 30 period from now.

```{python}
num_purchases = np.arange(6)  # Num of purchases
expected_probability_purchases = np.zeros(
    (len(num_purchases), len(params[0]), len(p1x_sample))
)
...
plt.gcf().suptitle(
    "Expected Probability of $y$ Purchases within 30 days from T.", fontweight="bold"
)
plt.show()
```
...
In this appendix, we list all the managerially relevant quantities and their expressions.
...
$$
\begin{align*}
L(r,\alpha,a,b\,|\,X =x,t_{x},T) &= \frac{B(a,b+x)}{B(a,b)}\frac{\Gamma(r+x)\alpha^{r}}{ \Gamma(r)(\alpha+T)^{r+x}} \\
&+ \delta_{x>0}\,\frac{B(a+1,b+x-1)}{B(a,b)}\frac{\Gamma(r+x)\alpha^{r}}{\Gamma(r)(\alpha+t_{x})^{r+x}}
\end{align*}
$$ {#eq-loglikelihood}
...
P(X(t)=x \mid r,\alpha,a,b) &= \frac{B(a,b+x)}{B(a,b)} \frac{\Gamma(r+x)}{\Gamma(r)x!}\left(\frac{\alpha}{\alpha+t}\right)^{r}\left(\frac{t}{\alpha+t}\right)^{x} \\
&+ \delta_{x>0}\,\frac{B(a+1,b+x-1)}{B(a,b)} \\
& \times \left[1-\left(\frac{\alpha}{\alpha+t}\right)^{r}\left\{\sum_{j=0}^{x-1}\frac{\Gamma(r+j)}{\Gamma(r)j!}\left(\frac{t}{ \alpha+t}\right)^{j}\right\} \right]
\end{align*}
$$ {#eq-P-X-t}
...
-   the probability that an individual with observed behavior $(X = x, t_x, T)$ is still active at time $T$,
-   the expected number of transactions in a future period of length $t$ for an individual with observed behavior $(X = x, t_x, T)$
-   and, an expression for the probability of making $y$ purchases in a future time period given the observed past behavior.

### $P(\text{alive at }T \mid X=x,t_{x},T)$
...
$$
P(\text{alive at }T\,|\,X=x,t_{x},T,r,\alpha,a,b) = 1 \bigg/ \left[1 + \delta_{x>0}\frac{a}{b+x-1}\left(\frac{\alpha + T}{\alpha + t_{x}}\right)^{r+x}\right].
$$ {#eq-P-alive-T}
...
$$
P(Y(t)=y \mid X=x,t_{x},T,r,\alpha,a,b)=\frac{\delta_{x>0,y=0}\,\mathsf{A}+ \mathsf{B}+\delta_{y>0}\,\mathsf{C}}{L(r,\alpha,a,b \mid X=x,t_{x},T)}
$$ {#eq-P-Y-t}
...
$$
\begin{align*}
\mathsf{C} &= \frac{B(a+1,b+x+y-1)}{B(a,b)} \\
&\times\left\{\frac{\Gamma(r+x)}{\Gamma(r)}\frac{\alpha^{r} }{(\alpha+T)^{r+x}}-\sum_{j=0}^{y-1}\frac{\Gamma(r+x+j)}{\Gamma(r)j!}\frac{ \alpha^{r}t^{j}}{(\alpha+T+t)^{r+x+j}}\right\}
\end{align*}
$$
