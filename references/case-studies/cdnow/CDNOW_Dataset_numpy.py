...
sampledata_file_path = os.path.join(
    current_dir, "..", "..", "data", "CDNOW", "CDNOW_sample.csv"
)
masterdata_file_path = os.path.join(
    current_dir, "..", "..", "data", "CDNOW", "CDNOW_master.csv"
)
...
CDNOW_sample = (
    ...
    .sort("CustID", "Date")
    .with_columns(
        (pl.col("CustID").cum_count().over("CustID") - 1).cast(pl.UInt16).alias("DoR")
    )
)
...
# The number of repeat transactions made by each customer in each period
p1x = np.sum(
    ((TransMAT[:, 2:] > 0) & (TransMAT[:, 2:] <= calwk)), axis=1, keepdims=True
)
...
CDNOW_master = (
    pl.scan_csv(
        source=masterdata_file_path,
        has_header=False,
        separator=",",
        schema={
            "CustID": pl.Int32,  # customer id
...
x = np.sum(
    ((master_TransMAT[:, 2:] > 0) & (master_TransMAT[:, 2:] <= 273)),
    axis=1,
    dtype="int16",
)
# Step 2: Create a mask to include only valid columns for each customer
mask = (master_TransMAT[:, 2:] > 0) & (
    master_TransMAT[:, 2:] <= 273
)  # Exclude ID Column & Trial
...
for i in range(1, 79):
    ...
    for j in range(1, MaxPurchNum + 1):
        isbuyer = np.where(
            (master_TransMAT[:, j] > (7 * (i - 1))) & (master_TransMAT[:, j] <= (7 * i))
        )[0]
...
for i in range(12):
    istrier = np.where(
        (master_TransMAT[:, 1] > (7 * i)) & (master_TransMAT[:, 1] <= (7 * (i + 1)))
    )[0]
...
for i in range(12):
    ...
    for j in range(len(istrier)):
        ...
        while not_done:
            ...
            not_done = (master_TransMAT[istrier[j], k + 1] > (7 * i)) & (
                master_TransMAT[istrier[j], k + 1] <= (7 * (i + 1))
            )
            k += 1
...
for i in range(12):
    for k in range(1, MaxPurchNum + 1):
        isbuyer = np.where(
            (master_TransMAT[:, k] > (7 * i)) & (master_TransMAT[:, k] <= (7 * (i + 1)))
        )[0]
