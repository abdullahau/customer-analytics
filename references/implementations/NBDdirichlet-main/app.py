...
cat_buyrate = st.number_input(
    "Category Buyer's Average Purchase Rate", value=default_cat_buyrate, format="%.2f"
)

# Add input for time period multiple
t_value = st.number_input(
    "Time Period Multiple", value=1, min_value=1, max_value=99, step=1
)

st.subheader("Brand Information")
num_brands = st.number_input(
    "Number of Brands", min_value=1, value=len(default_brand_name), step=1
)
...
for i in range(int(num_brands)):
    ...
    with col1:
        default_name = (
            default_brand_name[i] if i < len(default_brand_name) else f"Brand {i + 1}"
        )
    ...
    with col2:
        ...
        brand_share.append(
            st.number_input(
                f"Brand {i + 1} Market Share", value=default_share, format="%.2f"
            )
        )
    with col3:
        default_pen = (
            default_brand_pen_obs[i] if i < len(default_brand_pen_obs) else 0.1
        )
        brand_pen_obs.append(
            st.number_input(
                f"Brand {i + 1} Penetration", value=default_pen, format="%.2f"
            )
        )
...
if st.button("Run Dirichlet Model"):
    ...
    if "p_value" in locals():
        if p_value < 0.05:
            st.write(
                "The chi-square test suggests significant heterogeneity in the market (p < 0.05)."
            )
        else:
            st.write(
                "The chi-square test does not indicate significant heterogeneity in the market (p >= 0.05)."
            )

    if base_dobj.mape() > 10:
        st.write(
            "The MAPE is relatively high, suggesting considerable differences between observed and predicted values."
        )
    else:
        st.write(
            "The MAPE is relatively low, suggesting good agreement between observed and predicted values."
        )
    ...
    if t_value > 1:
        ...
        adj_dobj = Dirichlet(
            cat_pen, cat_buyrate, brand_share, brand_pen_obs, brand_name
        )
        ...
        adjusted_brand_metrics = pd.DataFrame(
            {
                ...
                "Adjusted Penetration": [
                    adj_dobj.brand_pen(j) for j in range(adj_dobj.nbrand)
                ],
                "Adjusted Buy Rate": [
                    adj_dobj.brand_buyrate(j) for j in range(adj_dobj.nbrand)
                ],
            }
        )
