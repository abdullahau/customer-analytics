...
def __(mo):
    ...
    period = mo.ui.dropdown(
        options=["Years", "Months"], value="Years", label="Choose Renewal Period:"
    )
...
def __(io, mo, np, period, upload):
    ...
    cohort_period, cohort_alive = np.loadtxt(
        file, dtype="f8", delimiter=",", unpack=True, skiprows=1
    )

    mo.md(
        f"#### **{period.value}**: {list(cohort_period)}\n #### **Customers Alive**: {list(cohort_alive)}"
    )
...
def __(cohort_period, mo, period):
    train_range = mo.ui.slider(
        steps=list(cohort_period),
        show_value=True,
        value=5,
...
def __(cohort_alive, cohort_period, mo, period, train_range):
    ...
    mo.md(
        f"#### **{period.value}**: {list(train_period)}\n #### **Customers Alive**: {list(train_alive)}"
    )
...
def __(minimize, np):
    def sBG(alive: np.array, period: np.array, guess=[1, 1]) -> tuple(float, float):  # type: ignore
        def log_likelihood(x):
            ...
            terms = (delta + period[2:] - 2) / (
                gamma + delta + period[2:] - 1
            )  # Define the sequence for t=2,3, ...
            p_churn[1:] = (
                np.cumprod(terms) * p_churn[0]
            )  # Calculate cumulative products for probabilities
...
def __(go, np, train_alive, train_period):
    def log_likelihood(gamma, delta):
        ...
        p_churn[0] = gamma / (gamma + delta)  # Define the base probability for t=1
        terms = (delta + train_period[2:] - 2) / (
            gamma + delta + train_period[2:] - 1
        )  # Define the sequence for t=2,3, ...
        p_churn[1:] = (
            np.cumprod(terms) * p_churn[0]
        )  # Calculate cumulative products for probabilities
    ...
    fig3d.update_layout(
        ...
        scene=dict(
            ...
            zaxis=dict(
                range=[-3600, -1400], tickvals=[-3400, -2900, -2400, -1900, -1400]
            ),
        ),
...
def __(delta_grid, gamma_grid, lls, np, plt):
    ...
    contour_plot = plt.contour(
        gamma_grid, delta_grid, lls, levels=levels, linewidths=0.75
    )
...
def __(mo):
    ...
    mo.vstack([gamma_slider, delta_slider], align="start")
...
def __(beta, delta_slider, gamma_slider, go, np):
    theta_x = np.linspace(
        beta.ppf(0, gamma_slider.value, delta_slider.value),
        beta.ppf(0.999, gamma_slider.value, delta_slider.value),
        100,
    )
    ...
    beta_dist.add_trace(
        go.Scatter(
            x=theta_x, y=pdf_y, mode="lines", line=dict(color="blue"), name="Beta PDF"
        )
    )
