...
def mh_gamma_chain(
    data, init_prior, hyper_prior, burnin=1000, num_iter=5000, thin=50, trace=100
):
    ...
    for step in range(1, burnin + num_iter + 1):
        ...
        if step % trace == 0:
            print(
                "step " + str(step) + " of total " + str(burnin + num_iter) + " steps"
            )

        one_mh_step_result = mh_gamma_step(
            data, current_param[0], current_param[1], hyper_prior
        )
...
if __name__ == "__main__":
    ...
    for i in range(mcmc_sample.shape[1]):
        ...
        print(
            f"overall mean={overall_mean}, first half mean={first_half_mean}, second half mean={second_half_mean}"
        )
        ...
        print(
            f"overall var={overall_var}, first half var={first_half_var}, second half var={second_half_var}"
        )
