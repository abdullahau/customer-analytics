...
def run_single_chain(
    data, init_param, chain_id=1, num_iter=3000, burn_in=1000, thin=50, trace=10
):

    N = data.shape[0]
    NUM_DRAWS = (num_iter - 1) / thin + 1

    latent_variables_draws = np.zeros(
        NUM_DRAWS,
        dtype=[
            ("lambda", float, N),
            ("mu", float, N),
    ...
    gamma_parameters_draws = np.zeros(
        NUM_DRAWS, dtype=[("r", float), ("alpha", float), ("s", float), ("beta", float)]
    )
    ...
    latent_variables["lambda"] = np.mean(data["x"]) / np.mean(
        [data["T.cal"][i] if data["t.x"][i] == 0 else data["t.x"][i] for i in range(N)]
    )
    latent_variables["tau"] = [
        data["t.x"][i] + 0.5 / latent_variables["lambda"][i] for i in range(N)
    ]
    latent_variables["alive"] = [
        int(latent_variables["tau"][i] > data["T.cal"][i]) for i in range(N)
    ]
    latent_variables["mu"] = [1 / latent_variables["tau"][i] for i in range(N)]

    for step in range(1, (num_iter + burn_in + 1)):
        if step % trace == 0:
            print(f"step {step} of chain {chain_id}")
        ...
        latent_variables = draw_latent_variables(
            data, latent_variables, gamma_parameters
        )
        gamma_parameters = draw_gamma_parameters(
            latent_variables, gamma_parameters, hyper_prior
        )

    return {
        "latent_param": latent_variables_draws,
        "gamma_param": gamma_parameters_draws,
    }
...
def draw_latent_variables(data, latent_variables, gamma_parameters):
    ...
    proposed_latent_variables["lambda"] = draw_lambda(
        r, alpha, x, T_cal, proposed_latent_variables["tau"]
    )
    proposed_latent_variables["mu"] = draw_mu(
        s, beta, proposed_latent_variables["tau"], T_cal
    )
    proposed_latent_variables["tau"] = draw_tau(
        proposed_latent_variables["lambda"], proposed_latent_variables["mu"], tx, T_cal
    )
...
def log_gamma_posterior(prior, hyper_prior, x):
    ...
    log_post = (
        n * (shape * math.log(rate) - math.lgamma(shape))
        + (shape - 1) * sum_log_x
        - rate * sum_x
        + (shape_hyper_1 - 1) * math.log(shape)
        - (shape * shape_hyper_2)
        + (rate_hyper_1 - 1) * math.log(rate)
        - (rate * rate_hyper_2)
    )
...
def slice_sampling_step(
    x, shape, rate, hyper_prior, steps=10, lower=1e-5, upper=100, w=1
):
...
def draw_gamma_parameters(latent_variables, gamma_parameters, all_hyper_prior):
    ...
    r_alpha_update = slice_sampling_step(
        latent_variables["lambda"],
        gamma_parameters["r"],
    ...
    s_beta_update = slice_sampling_step(
        latent_variables["mu"],
        gamma_parameters["s"],
        gamma_parameters["beta"],
        all_hyper_prior[4:],
    )
...
def draw_lambda(r, alpha, x, T_cal, tau):
    N = len(x)
    Lambda = [
        np.random.gamma(shape=r + x[i], scale=1 / (alpha + min(T_cal[i], tau[i])))
        for i in range(N)
    ]
    return Lambda
...
def draw_mu(s, beta, tau, T_cal):
    ...
    if len(alive_customers) > 0:
        mu[alive_customers] = [
            np.random.gamma(shape=s, scale=1 / (beta + T_cal[i]))
            for i in alive_customers
        ]

    if len(dead_customers) > 0:
        mu[dead_customers] = [
            np.random.gamma(shape=s + 1, scale=1 / (beta + tau[i]))
            for i in dead_customers
        ]
...
def draw_tau(Lambda, mu, tx, T_cal):
    ...
    p_alive = 1 / (
        1 + (mu / mu_lam) * (np.array([math.exp(i) for i in mu_lam_times_t_diff]) - 1)
    )
    ...
    if len(alive_customers) > 0:
        tau[alive_customers] = [
            T_cal[i] + np.random.exponential(1 / mu[i]) for i in range(N)
        ]
...
if __name__ == "__main__":
    if SIMULATE:
        ...
        hyper_prior = [1e-3] * 4
        test_gamma_slice_sampling = slice_sampling_step(
            sim_data, 0.5, 0.5, hyper_prior, steps=5
        )

        # slice sampling seems to be working.

        sim_parameters = {"r": 2.0, "a": 10.0, "s": 2.0, "b": 10.0}

        num_customer = 5000
        data = generate_pareto_nbd_data(sim_parameters, num_customer)

    data = pd.read_csv(
        os.path.join("..", "test_data", "pnbd_gibbs_sampling_sim_data.csv")
    )
