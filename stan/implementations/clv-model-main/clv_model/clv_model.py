...
class CLVModel:
    ...
    def is_fitted(self) -> bool:
        return self.value_model.is_fitted() and self.transactions_model.is_fitted()

    def predict(
        self, data: pandas.DataFrame, periods: int, discount_rate: Real
    ) -> pandas.DataFrame:
        """
        ...
        historic_clv = self._compute_historic_clv(
            rfm_df=data, discount_rate=discount_rate
        )
        future_clv = self._compute_future_clv(
            data=data, periods=periods, discount_rate=discount_rate
        )
    ...
    def _compute_future_clv(
        ...
        discounted_time = (
            periods if alpha == 1 else ((1 - alpha**periods) / (1 - alpha))
        )

        return (
            transactions.merge(values, on="id")
    ...
    def _compute_historic_clv(
        rfm_df: pandas.DataFrame, discount_rate: Real
    ) -> pandas.DataFrame:
        alpha = 1 / (1 + discount_rate)
        return rfm_df.assign(
            transaction_rate=lambda df: df.frequency / df["T"],
            discounted_time=lambda df: ((1 - alpha ** df["T"]) / (1 - alpha)).fillna(
                df["T"]
            ),
            clv=lambda df: df.transaction_rate * df.value * df.discounted_time,
        )[["id", "clv"]]
