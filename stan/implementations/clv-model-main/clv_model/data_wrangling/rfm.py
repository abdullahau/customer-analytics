...
def rfm(
    ...
    _check_column_presence(wanted=wanted_columns, present=set(transactions.columns))

    if observation_period_end is None:
        observation_period_end = pandas.to_datetime(transactions[date_col].max())
    ...
    return rf.merge(right=m, on="id", how="left").fillna(0)


def _determine_monetary_value(
    transactions: pandas.DataFrame,
) -> pandas.DataFrame:
    _check_column_presence(wanted={"value", "id"}, present=set(transactions.columns))

    return (
        transactions[["value", "id"]]
        .groupby("id", as_index=False, sort=False)
        .mean()
        .assign(value=lambda df: df.value.round(2))
    )
...
def _determine_recency_frequency(
    ...
    _check_column_presence(wanted={"date", "id"}, present=set(transactions.columns))

    return (
        transactions.groupby("id", sort=False)
        .date.agg(["min", "max", "count"])
        .reset_index()
        .assign(
            T=lambda df: (observation_period_end - df["min"]).apply(lambda x: x.n),
            recency=lambda df: (observation_period_end - df["max"]).apply(
                lambda x: x.n
            ),
...
def _group_by_period(transactions: pandas.DataFrame, period: str) -> pandas.DataFrame:
    _check_column_presence(wanted={"date", "id"}, present=set(transactions.columns))
...
def _check_column_presence(wanted: typing.Set[str], present: typing.Set[str]) -> None:
