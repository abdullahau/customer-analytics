...
class StanModelBase:
    def __init_subclass__(cls, model_name: str, **kwargs) -> None:
        cls._stan_model: Optional[pystan.StanModel] = _load_stan_model(model_name)
        cls.__model_name__: str = model_name
        cls.__parameters__ = {
            name for name, type_ in cls.__annotations__.items() if type_ == Parameter
        }
        cls.__annotations__.update(
            {parameter: Optional[numpy.ndarray] for parameter in cls.__parameters__}
        )
    ...
    def _compile_stan_model(cls) -> pystan.StanModel:
        ...
        with resources.open_text(
            STAN_MODELS_PACKAGE, f"{cls.__model_name__}.stan"
        ) as model_file:
            cls._stan_model = pystan.StanModel(
                model_file, model_name=cls.__model_name__
            )
    ...
    def fit(self, data: pandas.DataFrame, **kwargs) -> StanModelBase:
        ...
        data_dict = {**dict(data), "N": len(data)}
...
def _load_stan_model(model_name: str) -> Optional[pystan.StanModel]:
    try:
        with resources.open_binary(
            STAN_MODELS_PACKAGE, f"{model_name}.pkl"
        ) as model_file:
