...
class TestCLVModel(unittest.TestCase):
    ...
    def _get_model(
        self, global_mean: Optional[Real] = 1, mean_transaction_rate: Optional[Real] = 1
    ) -> CLVModel:
        ...
        return CLVModel(value_model=value_model, transactions_model=transactions_model)
    ...
    def test_predict_no_discount(self) -> None:
        ...
        expected = pandas.DataFrame(
            data={"id": [0, 1, 2], "clv": [0.5 + 0.5 + 1, 1 + 1 + 1, 4 + 1]}
        ).assign(clv=lambda df: df.clv.round(2))
    ...
    def test_predict_empty(self) -> None:
        ...
        actual = model.predict(
            data=pandas.DataFrame(columns={"id", "recency", "frequency", "T", "value"}),
            periods=1,
            discount_rate=0.15,
        )
    ...
    def test_predict_unfit(self) -> None:
        ...
        with self.assertRaises(ValueError) as error:
            model.predict(data=data, periods=1, discount_rate=0.2)
        self.assertEqual(
            str(error.exception),
            "Model must be fitted with a call to fit before predict can be called.",
        )
    ...
    def test_predict_bad_discount_rate(self) -> None:
        ...
        data = self._get_df()
        with self.assertRaises(ValueError) as error:
            model.predict(data=data, periods=1, discount_rate=1)
        self.assertEqual(str(error.exception), "Discount rate must be in [0,1).")
        ...
        with self.assertRaises(ValueError) as error:
            model.predict(data=data, periods=1, discount_rate=1729.01)
        self.assertEqual(str(error.exception), "Discount rate must be in [0,1).")
    ...
    def test_fit(self) -> None:
        ...
        self.assertEqual(model.value_model.global_mean, expected_mean_value)

        expected_transaction_rate = (3 / 2 + 2 / 2) / 2
        self.assertEqual(
            model.transactions_model.mean_transaction_rate, expected_transaction_rate
        )
