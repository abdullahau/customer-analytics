...
filepath = os.path.join(
    current_dir, "..", "data", "Donation-Incidence", "1995_cohort_binary.csv"
)
...
class Donation(object):
    ...
    def rfm_data(self) -> pl.LazyFrame:
        ...
        return (
            self.data.with_columns(
                pl.sum_horizontal(
                    pl.all().exclude(["ID", "1995", *[str(i) for i in valid_columns]])
                ).alias("P1X"),
                pl.sum_horizontal(
                    pl.all().exclude(["ID", "1995", *[str(i) for i in calib_columns]])
                ).alias("P2X"),
                pl.max_horizontal(
                    *[
                        ...
                        .then(int(year) - 1995)
                        .otherwise(None)
                        for year in calib_columns
                    ]
                ).alias("t_x"),
                pl.lit(self.calib_p).alias(
                    "np1x"
                ),  # n transaction opportunities in calibration period
                pl.lit(len(self.years) - 1 - self.calib_p).alias(
                    "np2x"
                ),  # n transaction opportunities  in validation period
            )
            .fill_null(0)
    ...
    def p1x_data(self) -> pl.LazyFrame:
        ...
        return (
            rfm_summary.group_by("P1X", "t_x", "np1x")
